...
| **onComplete**              | func                                                           | -            | Type def: function(): void \| {shouldRepeat: boolean, delay: number} On complete handler. It can be used to repeat the animation by returning an object with following props: `shouldRepeat` indicates if the animation should start over; `delay` specifies the delay before looping again in seconds. |
