...
| **easing**                  | string \| func      | easeOutCubic | Type def: easeOutCubic \| easeInCubic \| linear \| [easing func](http://www.gizma.com/easing/) &nbsp;Easing function to control the animation progress                                                                                                                                                  |
| **onComplete**              | func                | -            | Type def: () => void \| {shouldRepeat: boolean, delay: number} &nbsp;On complete handler. It can be used to repeat the animation by returning an object with following props: `shouldRepeat` indicates if the animation should start over; `delay` specifies the delay before looping again in seconds. |
| **formatter**               | func                | -            | Type def: (value: number) => number \| string \| node &nbsp;Function that formats the output value. It has the heights priority so all other formating options are ignored                                                                                                                              |
| **children**                | func                | -            | Type def: ({ value: number, reset: () => void }) => number \| string \| node &nbsp;Render function to render the count up value. Used only by the component                                                                                                                                             |
